...
<body>
	...
	<h1 id="election1">Welcome to The East African University Virtual Polling Station</h1>
		...
		else
		{
			...
			if ($conn){
				...
				if (empty($namErr))
				{
					...
					$sql="SELECT admission_number, student_name, email, password, status from student where admission_number=?";
					if ($stmt= mysqli_prepare($conn, $sql)){
						...
						if (mysqli_stmt_execute($stmt)){
							...
							if (mysqli_stmt_num_rows($stmt) == 1){
								...
								mysqli_stmt_bind_result($stmt, $param_admission, $param_student_name, $student_email, $password, $param_status);
								if (mysqli_stmt_fetch($stmt)){
									...
									if (password_verify($_POST["password"], $password))
									{
										...
										$_SESSION["user_admin"]= $param_admission;
										$state=$param_status;
										if ($state=1){
